...
def compute_cv(positions):
    ...
    p1, p2, p3, p4 = positions[jnp.array([2,11,12,13])] #if cis positions[jnp.array([1,6,7,8])]# if trans :
    #print(p1,p2,p3,p4)
    # Define vectors
    b1 = p2 - p1
    b2 = p3 - p2
...
mace_calc_dis = mace_off(model="large",dispersion=True,device="cuda:1")#XTB(method="GFN2-xTB")#
...
dyn = Langevin(atoms, timestep=.5 * units.fs, temperature_K=293, friction=0.1, trajectory=f'azob_dis_no_consbiased_{args.shift}_large_new_cv_trig.traj',fixcm=False)#Andersen(atoms, timestep=.5 * units.fs, temperature_K=3000, andersen_prob=0.01,trajectory='md.traj')#


N_steps = int(2e4)
dyn.run(N_steps)
traj = read(f'azob_dis_no_consbiased_{args.shift}_large_new_cv_trig.traj@2000::1')
cv_list=[]
energy_list=[]
with open(f"large_cv_energy_biased_dis_no_cons_{args.shift}_new_cv_trig.csv", 'w') as file:
...
for atoms in traj:
    ...
    with open(f"large_cv_energy_biased_dis_no_cons_{args.shift}_new_cv_trig.csv", 'a+') as file:
        writer =csv.writer(file)
        writer.writerow([cv,energy])
    cv_list.append(cv)
    energy_list.append(energy)
    
with open(f"large_mean_cv_energy_dis_no_cons_{args.shift}_new_cv_trig.csv", 'w') as file:
    writer =csv.writer(file)
    writer.writerow([x0,np.mean(np.array(cv_list)),-k_*(np.mean(np.array(cv_list))-(compute_cv(atoms1.positions)-5*args.shift)),np.mean(np.array(energy_list)),compute_cv(atoms1.positions)])
