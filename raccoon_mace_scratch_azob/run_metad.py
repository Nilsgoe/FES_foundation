#!/usr/bin/env python
"""Azobenzene gas-phase MetaD with the MACE from-scratch model.
...
MODEL_PATH = (
    ...
    "/azobenzene_dft_benchmark/models/azob_scratch_stagetwo.model"
)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--steps-per-chunk", type=int, default=4_000_000)
...
def build_calculator(device: str):
...
def run_nvt(atoms, nvt_steps: int, run_tag: str, outputs_dir: Path) -> None:
    ...
    print(f"NVT equilibration done ({nvt_steps} steps)")


def main() -> None:
    args = parse_args()
    spec = INDEX_MAP[args.system]
    run_tag = f"mace_scratch_azob_{args.system}_{args.cv_mode}"
    ...
    if args.chunk_id > 0 and not continue_run:
        raise FileNotFoundError(
            f"Continuation requested (chunk {args.chunk_id}) but missing {traj_path} or {bias_path}"
        )
    ...
    atoms.set_pbc(False)
    atoms.calc = build_calculator(args.device)
    ...
    print(f"Completed chunk {args.chunk_id}: {args.steps_per_chunk} MetaD steps")
