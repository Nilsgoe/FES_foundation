...
df = pd.read_csv("FES_UMD_malon.csv")
fes_umd = df['FES'].to_numpy()-.7
cv_umd = df['CV'].to_numpy()
o_file= "output_bias_1D.txt"
...
#                linestyles='-', colors='darkgray', linewidths=1.3)
...
plt.title("OH bond Malon",size=40)
plt.tight_layout()
plt.savefig('FES_UMD_metad_malon.png',dpi=300)
plt.show()
